...
impl Mixer {
    ...
    pub fn new(shift_hz: f64, rate: f64) -> Self {
        Self { phase: 0.0, step: std::f64::consts::TAU * shift_hz / rate }
    }
    ...
    pub fn process(&mut self, input: &[C32], out: &mut Vec<C32>) {
        ...
        for &x in input {
            ...
            out.push(x * C32::new(c as f32, s as f32));
            self.phase += self.step;
            // Wrap every sample so the argument to sin_cos stays small and
            // precision does not decay over a long capture.
